...
<div class="p-4 md:p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-3xl font-semibold text-primary-color">Good {{ $greeting }}, {{ auth()->user()->first_name }}!</h1>
                <p class="text-base-content/60 text-secondary-color">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            <a href="{{ route('tasks.index') }}" class="card bg-base-100 shadow hover:shadow-lg transition-shadow">
                <div class="card-body p-3">
                    <div class="flex items-center gap-3 justify-between flex-row-reverse">
                        <div class="size-8 full-radius rounded-lg bg-primary/10 flex items-center justify-center">
                            <span class="icon-[tabler--list-check] size-5 text-primary"></span>
                        </div>
                        <div>
                            <p class="text-2xl font-bold">{{ $stats['total_tasks'] ?? 0 }}</p>
                            <p class="text-xs text-base-content/60 text-secondary-color">Total Tasks</p>
                        </div>
                    </div>
                </div>
            </a>
            <div class="card bg-base-100 shadow">
                <div class="card-body p-3">
                    <div class="flex items-center gap-3 justify-between flex-row-reverse">
                        <div class="size-8 full-radius rounded-lg bg-warning/10 flex items-center justify-center">
                            <span class="icon-[tabler--clock] size-5 text-warning"></span>
                        </div>
                        <div>
                            <p class="text-2xl font-bold">{{ $stats['pending_tasks'] ?? 0 }}</p>
                            <p class="text-xs text-base-content/60 text-secondary-color">Pending</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="card bg-base-100 shadow">
                <div class="card-body p-3">
                    <div class="flex items-center gap-3 justify-between flex-row-reverse">
                        <div class="size-8 full-radius rounded-lg bg-info/10 flex items-center justify-center">
                            <span class="icon-[tabler--progress] size-5 text-info"></span>
                        </div>
                        <div>
                            <p class="text-2xl font-bold">{{ $stats['in_progress_tasks'] ?? 0 }}</p>
                            <p class="text-xs text-base-content/60 text-secondary-color">In Progress</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="card bg-base-100 shadow">
                <div class="card-body p-3">
                    <div class="flex items-center gap-3 justify-between flex-row-reverse">
                        <div class="size-8 full-radius rounded-lg bg-success/10 flex items-center justify-center">
                            <span class="icon-[tabler--circle-check] size-5 text-success"></span>
                        </div>
                        <div>
                            <p class="text-2xl font-bold">{{ $stats['completed_tasks'] ?? 0 }}</p>
                            <p class="text-xs text-base-content/60 text-secondary-color">Completed</p>
                        </div>
                    </div>
                </div>
            </div>
            <a href="{{ route('ideas.index') }}" class="card bg-base-100 shadow hover:shadow-lg transition-shadow">
                <div class="card-body p-3">
                    <div class="flex items-center gap-3 justify-between flex-row-reverse">
                        <div class="size-8 full-radius rounded-lg bg-accent/10 flex items-center justify-center">
                            <span class="icon-[tabler--bulb] size-5 text-accent"></span>
                        </div>
                        <div>
                            <p class="text-2xl font-bold">{{ $stats['ideas_count'] ?? 0 }}</p>
                            <p class="text-xs text-base-content/60 text-secondary-color">Ideas</p>
                        </div>
                    </div>
                </div>
            </a>
                ...
                <div class="card bg-base-100 shadow">
                    <div class="card-body">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <a href="/tasks" class="btn btn-ghost btn-sm btn-bg-none text-primary-color font-semibold">View All</a>
                        </div>
            ...
            <div class="card bg-base-100 shadow">
                <div class="card-body space-y-4">
                    ...
                    <div class="flex bg-base-200 rounded-lg p-1">
                        <button
                            class="tab-btn flex-1 py-2 text-sm font-medium rounded-md bg-base-100 shadow"
                            data-tab="upcoming">
                            Upcoming Events
                        </button>
                        <button
                            class="tab-btn flex-1 py-2 text-sm font-medium rounded-md text-base-content/60"
                            data-tab="activity">
                            Activity
                        </button>
                    </div>
                    ...
                    <div id="tab-activity" class="tab-content hidden">
                        @if(empty($activities))
                        <div class="flex flex-col items-center justify-center py-10 text-base-content/60">
                            <span class="icon-[tabler--activity] size-8 mb-2"></span>
    ...
    document.querySelectorAll('.tab-btn').forEach(button => {
        button.addEventListener('click', () => {
            ...
            document.querySelectorAll('.tab-btn').forEach(btn => {
                btn.classList.remove('bg-base-100', 'shadow');
