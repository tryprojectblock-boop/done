...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <!-- Header -->
    <div>
        <h1 class="text-2xl font-bold text-base-content">Edit Coupon</h1>
        <p class="text-base-content/60">Update coupon details</p>
    </div>
    ...
    <form action="{{ route('backoffice.coupons.update', $coupon) }}" method="POST">
        ...
        <div class="card bg-base-100 shadow mb-6">
            <div class="card-body">
                ...
                <div class="alert alert-info mt-4">
                    ...
                    <span>This coupon has been used <strong>{{ $coupon->usage_count }}</strong> time(s).</span>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="form-control">
                    <label class="label cursor-pointer justify-start gap-4">
                        <input type="checkbox" name="is_active" value="1" class="toggle toggle-primary" {{ old('is_active', $coupon->is_active) ? 'checked' : '' }} />
                        <div>
        ...
        <div class="card bg-base-100 shadow mb-6">
            <div class="card-body">
                ...
                <p class="text-sm text-base-content/60 mb-4">Preview of how prices will look with a <strong id="preview-discount">{{ $coupon->discount_percent }}</strong>% discount applied</p>
                ...
                <div class="overflow-x-auto">
                    <table class="table table-sm">
                        ...
                        <tbody id="discount-preview-body">
                            @php
                                $multiplier = 1 - ($coupon->discount_percent / 100);
                            @endphp
                            ...
                            <tr data-plan-id="{{ $plan->id }}"
                                ...
                                <td>
                                    ...
                                    <span class="text-success font-medium discounted-price-1">${{ number_format($plan->price_1_month * $multiplier, 2) }}</span>
                                </td>
                                <td>
                                    <span class="text-base-content/40 line-through text-xs">${{ number_format($plan->price_3_month, 2) }}</span>
                                    <span class="text-success font-medium discounted-price-3">${{ number_format($plan->price_3_month * $multiplier, 2) }}</span>
                                </td>
                                <td>
                                    <span class="text-base-content/40 line-through text-xs">${{ number_format($plan->price_6_month, 2) }}</span>
                                    <span class="text-success font-medium discounted-price-6">${{ number_format($plan->price_6_month * $multiplier, 2) }}</span>
                                </td>
                                <td>
                                    <span class="text-base-content/40 line-through text-xs">${{ number_format($plan->price_12_month, 2) }}</span>
                                    <span class="text-success font-medium discounted-price-12">${{ number_format($plan->price_12_month * $multiplier, 2) }}</span>
                                </td>
                                <td>
                                    <span class="text-base-content/40 line-through text-xs">${{ number_format($plan->price_3_year, 2) }}</span>
                                    <span class="text-success font-medium discounted-price-36">${{ number_format($plan->price_3_year * $multiplier, 2) }}</span>
                                </td>
                                <td>
                                    <span class="text-base-content/40 line-through text-xs">${{ number_format($plan->price_5_year, 2) }}</span>
                                    <span class="text-success font-medium discounted-price-60">${{ number_format($plan->price_5_year * $multiplier, 2) }}</span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
