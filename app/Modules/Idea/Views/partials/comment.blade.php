@php
    $user = auth()->user();
    $editEditorId = 'edit-idea-editor-' . $comment->id;
    $replyEditorId = 'reply-idea-editor-' . $comment->id;
@endphp

<div class="flex gap-3" id="idea-comment-{{ $comment->id }}">
    <div class="avatar">
    ...
    <div class="flex-1">
        <div class="bg-base-200 rounded-lg p-3">
            ...
            <div class="prose prose-sm max-w-none" id="idea-comment-content-{{ $comment->id }}">
                {!! $comment->content !!}
            </div>

            <!-- Edit Form (hidden by default) -->
            <div id="idea-comment-edit-{{ $comment->id }}" class="hidden mt-2">
                <form action="{{ route('ideas.comments.update', $comment) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="flex justify-end gap-2 mt-2">
                        <button type="button" class="btn btn-ghost btn-xs" onclick="cancelIdeaEdit({{ $comment->id }})">Cancel</button>
        ...
        <div class="flex items-center gap-2 mt-1">
            <button type="button" class="inline-flex items-center gap-1 text-xs text-base-content/50 hover:text-primary transition-colors" onclick="toggleIdeaReply({{ $comment->id }}, '{{ $replyEditorId }}')">
                <span class="icon-[tabler--message] size-4"></span>
                Reply
            </button>
            @if($comment->canEdit($user))
                <button type="button" class="inline-flex items-center gap-1 text-xs text-base-content/50 hover:text-primary transition-colors" onclick="editIdeaComment({{ $comment->id }}, '{{ $editEditorId }}')">
                    <span class="icon-[tabler--edit] size-4"></span>
                    Edit
                </button>
            @endif
            @if($comment->canDelete($user))
                <form action="{{ route('ideas.comments.destroy', $comment) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
        ...
        <div id="idea-reply-form-{{ $comment->id }}" class="hidden mt-2">
            <form action="{{ route('ideas.comments.store', $idea) }}" method="POST">
                ...
                <div class="flex justify-end gap-2 mt-2">
                    <button type="button" class="btn btn-ghost btn-xs" onclick="toggleIdeaReply({{ $comment->id }}, '{{ $replyEditorId }}')">Cancel</button>
        ...
        @if($comment->replies->isNotEmpty())
            <div class="ml-4 mt-3 space-y-3 border-l-2 border-base-300 pl-4">
                @foreach($comment->replies as $reply)
                    @include('idea::partials.comment', ['comment' => $reply, 'idea' => $idea])
                @endforeach
            </div>
        @endif
    </div>
...
@push('scripts')
<script>
function editIdeaComment(id, editorId) {
    document.getElementById('idea-comment-content-' + id).classList.add('hidden');
    document.getElementById('idea-comment-edit-' + id).classList.remove('hidden');
    // Initialize Quill editor if not already initialized
    initIdeaMiniQuill(editorId);
}

function cancelIdeaEdit(id) {
    document.getElementById('idea-comment-content-' + id).classList.remove('hidden');
    document.getElementById('idea-comment-edit-' + id).classList.add('hidden');
}

function toggleIdeaReply(id, editorId) {
    const form = document.getElementById('idea-reply-form-' + id);
...
function initIdeaMiniQuill(editorId) {
