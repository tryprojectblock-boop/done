...
<div class="min-h-screen p-4 md:p-6">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-base-content">Edit Standup</h1>
                <p class="text-base-content/60">{{ $entry->standup_date->format('l, F j, Y') }}</p>
            </div>
        </div>
        ...
        <form action="{{ route('standups.update', ['workspace' => $workspace, 'entry' => $entry]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="card bg-base-100 shadow">
                <div class="card-body space-y-6">
                    @php
                        $existingResponses = collect($entry->responses)->keyBy('question_id');
                    @endphp

                    @foreach($template->getOrderedQuestions() as $question)
                        @if($question['type'] !== 'mood')
                            @php
                                $existingAnswer = $existingResponses[$question['id']]['answer'] ?? '';
                            @endphp
                            <div class="form-control">
                                ...
                                >{{ old('responses.' . $question['id'], $existingAnswer) }}</textarea>
                            </div>
                        @endif
                    @endforeach
                            ...
                            @foreach($moodOptions as $option)
                                <label class="cursor-pointer">
                                    <input type="radio" name="mood" value="{{ $option['value'] }}"
                                           class="hidden peer"
                                           {{ old('mood', $entry->mood?->value) === $option['value'] ? 'checked' : '' }} />
